...
M.launch = function() {
	...
	M.init = function(div) {
		...
		M.tradeTooltip = function(id, n) {
			return function() {
				...
				var str = '<div style="padding:8px 4px;min-width:128px;text-align:center;font-size:11px;">' + '<div style="font-size:9px;opacity:0.6;">Stock: <b' + ((!buyOrSell && stock == 0) ? ' class="red"' : '') + '>' + Beautify(stock) + '</b>/<b' + ((buyOrSell && stock >= maxStock) ? ' class="red"' : '') + '>' + Beautify(maxStock) + '</b></div>' + '<div class="line"></div>' + '<div>' + (buyOrSell ? 'Buy' : 'Sell') + ' <b>' + Beautify(n) + '</b>x <div class="icon" style="pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -16px -14px;vertical-align:middle;background-position:' + (-me.icon[0] * 48) + 'px ' + (-me.icon[1] * 48) + 'px;"></div> ' + me.name + '</div>' + '<div>for <b>$' + Beautify(val, 2) + '</b> each</div>' + (overhead > 1 ? ('<div style="font-size:9px;opacity:0.6;">(+<b>' + Beautify((overhead - 1) * 100, 2) + '%</b> overhead)</div>') : '') + '<div class="line"></div>' + '<div style="font-size:9px;opacity:0.6;font-weight:bold;">' + (buyOrSell ? 'you spend' : 'you earn') + ':</div>' + '<div><b class="hasTinyCookie ' + (n <= 0 ? 'gray' : (Game.cookies >= cost * n || !buyOrSell) ? 'green' : 'red') + '">' + Beautify(cost * n) + '</b></div>' + (n > 0 ? ('<div style="font-size:9px;opacity:0.6;font-weight:bold;">($' + Beautify(val * overhead * n, 2) + ')</div>' + '<div style="font-size:9px;opacity:0.6;font-weight:bold;">(' + Game.sayTime(val * overhead * n * Game.fps, -1) + ' of CpS)</div>') : '') + (((me.last == 1 && !buyOrSell) || (me.last == 2 && buyOrSell)) ? '<div class="line"></div><div class="red">You cannot buy and sell this stock in the same tick.</div>' : '') + '</div>';
				return str;
			};
		}
		...
		M.officeTooltip = function() {
			return function() {
				...
				var icon = me.icon || [0, 0];
				var str = '<div style="padding:8px 4px;min-width:350px;">' + '<div class="icon" style="float:left;margin-left:-8px;margin-top:-8px;background-position:' + (-icon[0] * 48) + 'px ' + (-icon[1] * 48) + 'px;"></div>' + '<div class="name">' + me.name + ' <span style="font-size:11px;opacity:0.6;">[Level ' + (M.officeLevel + 1) + ' offices]</span></div>' + '<div class="line"></div><div class="description" style="font-size:11px;">' + me.desc + '</div>' + (me.cost ? ('<div class="line"></div><div style="font-size:11px;padding-left:24px;position:relative;">' + '<div id="bankOfficeIcon" class="icon" style="position:absolute;left:0px;top:6px;pointer-events:none;display:inline-block;transform:scale(0.5);margin:-16px -18px -16px -14px;vertical-align:middle;background-position:' + (-11 * 48) + 'px ' + (-0 * 48) + 'px;"></div>' + 'Upgrading will cost you <b class="' + (Game.Objects['Cursor'].amount >= me.cost[0] ? 'green' : 'red') + '">' + me.cost[0] + ' cursors</b>.<br>' + 'Upgrading requires <b class="' + (Game.Objects['Cursor'].level >= me.cost[1] ? 'green' : 'red') + '">level ' + me.cost[1] + ' cursors</b>.' + '</div>') : '') + '</div>';
				return str;
			};
		}
		M.brokers = 0;
		...
		M.brokersTooltip = function() {
			return function() {
				var icon = [1, 33];
				var str = '<div style="padding:8px 4px;min-width:350px;">' + '<div class="icon" style="float:left;margin-left:-8px;margin-top:-8px;background-position:' + (-icon[0] * 48) + 'px ' + (-icon[1] * 48) + 'px;"></div>' + '<div class="name">Stockbrokers <span style="font-size:11px;opacity:0.6;">(you have ' + Beautify(M.brokers) + ')</span></div>' + '<div class="line"></div><div class="description" style="font-size:11px;">' + 'A nice broker to trade more cookies.<br>' + '&bull; Buying goods normally incurs overhead costs of <b>20% extra</b>. Each broker you hire reduces that cost by <b>5%</b>.<br>' + '&bull; Current overhead costs thanks to your ' + Beautify(M.brokers) + ' broker' + (M.brokers == 1 ? '' : 's') + ': <b>+' + Beautify(20 * Math.pow(0.95, M.brokers), 2) + '%</b><br>' + '&bull; Buying a broker costs <b class="hasTinyCookie ' + (Game.cookies >= M.getBrokerPrice() ? 'green' : 'red') + '">20 minutes</b> of CpS (that\'s $1200).<br>' + '&bull; Maximum number of brokers you can own: <b class="' + (M.brokers < M.getMaxBrokers() ? 'green' : 'red') + '">' + Beautify(M.getMaxBrokers()) + '</b> (the highest amount of grandmas you\'ve owned this run, divided by 10, plus your grandma level)<br>' + '<q>Brokers are Wall Street-class grandmas versed in the ways of finance. Stockbroker grandmas work hard and play hard, and will fight telephone in hand to get your clients the best possible deals - with a sizeable profit margin for you, of course.</q>' + '<div class="line"></div><div style="font-size:11px;text-align:center;">' + 'Hiring a new broker will cost you <b class="hasTinyCookie ' + (Game.cookies >= M.getBrokerPrice() ? 'green' : 'red') + '">' + Beautify(M.getBrokerPrice()) + ' cookies</b>.' + '</div>' + '</div>' + '</div>';
				return str;
			};
		}
		M.loanTypes = [
		...
		M.loanTooltip = function(id) {
			return function() {
				...
				var str = '<div style="padding:8px 4px;min-width:350px;">' + '<div class="name">Take out ' + loan[0] + '</div>' + '<div class="line"></div><div class="description" style="font-size:11px;">' + 'By taking this loan, you will get <b class="green">+' + Math.round((loan[1] - 1) * 100) + '%</b> CpS for the next <b>' + Game.sayTime(60 * loan[2] * Game.fps) + '</b>.<br>' + 'However, you will get <b class="red">' + Math.round((loan[3] - 1) * 100) + '%</b> CpS for the next <b>' + Game.sayTime(60 * loan[4] * Game.fps) + '</b> after that.<br>' + 'You must also pay an immediate downpayment of <b class="hasTinyCookie red">' + Beautify(Game.cookies * loan[5]) + '</b> (<b>' + (loan[5] * 100) + '%</b> of your current bank).<br>' + '<q>' + loan[6] + '</q>' + '</div>';
				return str;
			};
		}
		...
		for (var i = 0; i < M.goodsById.length; i++) {
			...
			str += '<div class="bankGood" id="bankGood-' + me.id + '">' + '<div ' + Game.getDynamicTooltip('Game.ObjectsById[' + M.parent.id + '].minigame.goodTooltip(' + me.id + ')', 'this') + '>' + '<div class="icon" style="z-index:20;pointer-events:none;position:absolute;left:0px;top:0px;transform:scale(0.5);margin:-16px -16px;background-position:' + (-me.icon[0] * 48) + 'px ' + (-me.icon[1] * 48) + 'px;"></div>' + '<div class="bankSymbol" style="margin:1px 0px;display:block;padding:2px 0px;width:100%;overflow:hidden;white-space:nowrap;">' + me.symbol + ' <span id="bankGood-' + me.id + '-sym" class="bankSymbolNum">-.--%</span></div>' + '<div class="bankViewHide bankSimpleButton" id="bankGood-' + me.id + '-viewHide">Hide</div>' + '<div class="bankSymbol" style="margin:1px 0px;display:block;font-size:10px;width:100%;background:linear-gradient(to right,transparent,#333,#333,transparent);padding:2px 0px;overflow:hidden;white-space:nowrap;">value: <span style="font-weight:bold;color:#fff;" id="bankGood-' + me.id + '-val">-</span></div>' + '<div class="bankSymbol" style="margin:1px 0px;display:block;font-size:10px;width:100%;background:linear-gradient(to right,transparent,#333,#333,transparent);padding:2px 0px;overflow:hidden;white-space:nowrap;" id="bankGood-' + me.id + '-stockBox">stock: <span style="font-weight:bold;" id="bankGood-' + me.id + '-stock">-</span><span style="font-weight:bold;" id="bankGood-' + me.id + '-stockMax">/-</span></div>' + '</div>' + '<div style="position:relative;white-space:nowrap;">' + '<div style="padding:3px 2px;width:22px;" class="bankSymbol">Buy</div>' + '<div class="bankButton bankButtonBuy" id="bankGood-' + me.id + '_1" ' + Game.getDynamicTooltip('Game.ObjectsById[' + M.parent.id + '].minigame.tradeTooltip(' + me.id + ',1)', 'this') + '>1</div>' + '<div class="bankButton bankButtonBuy" id="bankGood-' + me.id + '_10" ' + Game.getDynamicTooltip('Game.ObjectsById[' + M.parent.id + '].minigame.tradeTooltip(' + me.id + ',10)', 'this') + '>10</div>' + '<div class="bankButton bankButtonBuy" id="bankGood-' + me.id + '_100" ' + Game.getDynamicTooltip('Game.ObjectsById[' + M.parent.id + '].minigame.tradeTooltip(' + me.id + ',100)', 'this') + '>100</div>' + '<div style="width:28px;" class="bankButton bankButtonBuy" id="bankGood-' + me.id + '_Max" ' + Game.getDynamicTooltip('Game.ObjectsById[' + M.parent.id + '].minigame.tradeTooltip(' + me.id + ',10000)', 'this') + '>Max</div>' + '<br>' + '<div style="padding:3px 2px;width:22px;" class="bankSymbol">Sell</div>' + '<div class="bankButton bankButtonSell" id="bankGood-' + me.id + '_-1" ' + Game.getDynamicTooltip('Game.ObjectsById[' + M.parent.id + '].minigame.tradeTooltip(' + me.id + ',-1)', 'this') + '>1</div>' + '<div class="bankButton bankButtonSell" id="bankGood-' + me.id + '_-10" ' + Game.getDynamicTooltip('Game.ObjectsById[' + M.parent.id + '].minigame.tradeTooltip(' + me.id + ',-10)', 'this') + '>10</div>' + '<div class="bankButton bankButtonSell" id="bankGood-' + me.id + '_-100" ' + Game.getDynamicTooltip('Game.ObjectsById[' + M.parent.id + '].minigame.tradeTooltip(' + me.id + ',-100)', 'this') + '>100</div>' + '<div style="width:28px;" class="bankButton bankButtonSell" id="bankGood-' + me.id + '_-All" ' + Game.getDynamicTooltip('Game.ObjectsById[' + M.parent.id + '].minigame.tradeTooltip(' + me.id + ',-10000)', 'this') + '>All</div>' + '</div>' + '</div>';
		}
		str += '</div>';
		...
		div.innerHTML = str;
		var str = '';
		str += '<div style="position:absolute;left:2px;top:2px;z-index:10;">' + '<div id="bankGraphLines" class="bankSimpleButton" style="background:rgba(0,0,0,0.5);padding:2px;border-radius:4px;">Line style</div>' + '<div id="bankGraphCols" class="bankSimpleButton" style="background:rgba(0,0,0,0.5);padding:2px;border-radius:4px;">Color mode</div>' + (Game.sesame ? '<div id="bankCheatSpeed" class="bankSimpleButton" style="background:rgba(0,0,0,0.5);padding:2px;border-radius:4px;">Toggle speed</div>' : '') + '</div>' + '<div style="font-family:Arial Black;font-weight:40px;letter-spacing:2px;opacity:0.15;font-weight:bold;position:absolute;left:6px;bottom:6px;z-index:10;pointer-events:none;">DOUGH JONES INDEX</div>';
		for (var i = 0; i < M.goodsById.length; i++) {
			var me = M.goodsById[i];
		...
		M.graph = div;
		M.graphCtx = M.graph.getContext('2d', {
			alpha: false
		});
		...
		for (var i = 0; i < M.goodsById.length; i++) {
			...
			AddEvent(l('bankGood-' + i), 'mouseover', function(i) {
				return function(e) {
					if (M.hoverOnGood != i) {
						M.hoverOnGood = i;
						M.toRedraw = 2;
			...
			AddEvent(l('bankGood-' + i), 'mouseout', function(i) {
				return function(e) {
					if (M.hoverOnGood == i) {
						M.hoverOnGood = -1;
						M.toRedraw = 2;
			...
			AddEvent(l('bankGood-' + i + '-viewHide'), 'click', function(i) {
				return function(e) {
			...
			AddEvent(l('bankGood-' + i + '_1'), 'click', function(i) {
				return function(e) {
					if (M.buyGood(i, 1)) Game.SparkleOn(e.target);
				}
			}(i));
			AddEvent(l('bankGood-' + i + '_-1'), 'click', function(i) {
				return function(e) {
					if (M.sellGood(i, 1)) Game.SparkleOn(e.target);
				}
			}(i));
			AddEvent(l('bankGood-' + i + '_10'), 'click', function(i) {
				return function(e) {
					if (M.buyGood(i, 10)) Game.SparkleOn(e.target);
				}
			}(i));
			AddEvent(l('bankGood-' + i + '_-10'), 'click', function(i) {
				return function(e) {
					if (M.sellGood(i, 10)) Game.SparkleOn(e.target);
				}
			}(i));
			AddEvent(l('bankGood-' + i + '_100'), 'click', function(i) {
				return function(e) {
					if (M.buyGood(i, 100)) Game.SparkleOn(e.target);
				}
			}(i));
			AddEvent(l('bankGood-' + i + '_-100'), 'click', function(i) {
				return function(e) {
					if (M.sellGood(i, 100)) Game.SparkleOn(e.target);
				}
			}(i));
			AddEvent(l('bankGood-' + i + '_Max'), 'click', function(i) {
				return function(e) {
					if (M.buyGood(i, 10000)) Game.SparkleOn(e.target);
				}
			}(i));
			AddEvent(l('bankGood-' + i + '_-All'), 'click', function(i) {
				return function(e) {
	...
	M.save = function() {
		...
		var str = '' + parseInt(M.officeLevel) + ':' + parseInt(M.brokers) + ':' + parseInt(M.graphLines) + ':' + parseFloat(M.profit) + ':' + parseInt(M.graphCols) + ':' + ' ';
